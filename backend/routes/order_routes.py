...
def get_order():
    ...
    user_orders = User.from_bson(user_collection.get_by_email(email)).orders
...
@order_route.route('/list_order', methods=['LIST'])
@login_required
def list_order():
    email: str = current_user.email
    user_orders = User.from_bson(user_collection.get_by_email(email)).orders
    orders_json = [order.to_bson for order in user_orders]
